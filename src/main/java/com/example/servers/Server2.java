...
public class Server2 {
    private static final int ADMIN_PORT = 5002; 
    private static final int CLIENT_PORT = 5012; 
    private static final int SERVER_PORT = 5022; 
    private static final List<Integer> OTHER_SERVER_PORTS = Arrays.asList(5021, 5023); 
    private static boolean isClientPortActive = false;

    public static void main(String[] args) {
        new Thread(Server2::listenToAdmin).start(); // Admin için dinleyici başlat
    }

    private static void listenToAdmin() {
        try (ServerSocket serverSocket = new ServerSocket(ADMIN_PORT)) {
            System.out.println("Server2 Admin için dinliyor (Port: " + ADMIN_PORT + ")");
            while (true) {
                try (Socket socket = serverSocket.accept()) {
                    ...
                    if (config.getMethod() == ConfigurationOuterClass.MethodType.STRT && !isClientPortActive) {
                        ...
                        new Thread(Server2::listenToClients).start(); // Client portunu dinlemeye başla
                        new Thread(Server2::listenToServers).start(); // Sunucular için portu dinlemeye başla
                    } else if (config.getMethod() == ConfigurationOuterClass.MethodType.STOP && isClientPortActive) {
                        System.out.println("Server2 durduruldu!");
    ...
    private static void listenToClients() {
        try (ServerSocket serverSocket = new ServerSocket(CLIENT_PORT)) {
            System.out.println("Server2 Client için dinliyor (Port: " + CLIENT_PORT + ")");
    ...
    private static void listenToServers() {
        try (ServerSocket serverSocket = new ServerSocket(SERVER_PORT)) {
            System.out.println("Server2 Diğer sunucular için dinliyor (Port: " + SERVER_PORT + ")");
            while (isClientPortActive) {
                try (Socket socket = serverSocket.accept()) {
                    InputStream input = socket.getInputStream();
                    SubscriberOuterClass.Subscriber subscriber = SubscriberOuterClass.Subscriber.parseFrom(input);
